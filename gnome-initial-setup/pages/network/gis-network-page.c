...
struct _GisNetworkPagePrivate {
  ...
  /* TRUE if the page should always be shown (for debugging purposes). */
...
sync_complete (GisNetworkPage *page)
{
  ...
  if (priv->always_show || priv->ever_shown) {
    visible = TRUE;
  } else if (!has_device) {
...
gis_network_page_constructed (GObject *object)
{
  ...
  priv->always_show = g_getenv ("GIS_ALWAYS_SHOW_NETWORK_PAGE") != NULL;
